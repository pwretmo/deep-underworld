...
export class SirenSkull {
  ...
  _buildModel() {
    ...
    for (const [tierName, profile] of Object.entries(SIREN_TIER_PROFILE)) {
      ...
      const dist = tierName === 'near' ? 0 : tierName === 'medium' ? SIREN_LOD_NEAR_DISTANCE : SIREN_LOD_MEDIUM_DISTANCE;
...
totalEmissiveRadiance += vec3(0.22, 0.08, 0.12) * vEdgeFlicker * (0.5 + uPulse * 0.5);`
  ...
  _buildTier(profile, tierName) {
    ...
    const detailScale = tierName === 'near' ? 1 : tierName === 'medium' ? 0.65 : 0.2;

    const boneMaterial = this._createBoneMaterial(useFarMat, detailScale);
    const sutureMaterial = useFarMat
      ? new THREE.MeshStandardMaterial({ color: 0x262018, emissive: 0x140e0c, emissiveIntensity: 0.15 })
      : new THREE.MeshPhysicalMaterial({ color: 0x2a241d, roughness: 0.82, metalness: 0.05, emissive: 0x120c0a, emissiveIntensity: 0.2 });
    const socketMaterial = useFarMat
      ? new THREE.MeshStandardMaterial({ color: 0x09090a, emissive: 0x200b12, emissiveIntensity: 0.35 })
      : new THREE.MeshPhysicalMaterial({ color: 0x08080a, emissive: 0x341018, emissiveIntensity: 0.6, roughness: 1.0, metalness: 0.0 });
    const eyeMaterial = useFarMat
      ? new THREE.MeshStandardMaterial({ color: 0x93253f, emissive: 0x8c1d34, emissiveIntensity: 1.0 })
      : new THREE.MeshPhysicalMaterial({ color: 0xc42a4a, emissive: 0xb4203d, emissiveIntensity: 1.6, roughness: 0.06, metalness: 0.0, transmission: 0.35, thickness: 0.2 });
    const membraneMaterial = this._createMembraneMaterial(useFarMat, tierName === 'near' ? 0.44 : tierName === 'medium' ? 0.36 : 0.22);
    const hornMaterial = useFarMat
      ? new THREE.MeshStandardMaterial({ color: 0x413229, emissive: 0x130f0b, emissiveIntensity: 0.12 })
      : new THREE.MeshPhysicalMaterial({ color: 0x4e3f32, roughness: 0.34, metalness: 0.1, emissive: 0x1c140f, emissiveIntensity: 0.22 });
    const ghostMaterial = useFarMat
      ? new THREE.MeshStandardMaterial({ color: 0x7ef8ff, emissive: 0x49d7ec, emissiveIntensity: 0.8, transparent: true, opacity: 0.8 })
      : new THREE.MeshPhysicalMaterial({ color: 0x8ff4ff, emissive: 0x53dbf0, emissiveIntensity: 1.8, roughness: 0.05, metalness: 0.0, transmission: 0.8, thickness: 0.15, transparent: true, opacity: 0.88 });

    const skull = profile.skullSegs[1] === 0
      ? new THREE.Mesh(new THREE.OctahedronGeometry(0.78, profile.skullSegs[0]), boneMaterial)
      : new THREE.Mesh(new THREE.SphereGeometry(0.5, profile.skullSegs[0], profile.skullSegs[1]), boneMaterial);
    ...
    if (tierName !== 'far') {
      for (let i = 0; i < 3; i++) {
        ...
        for (let s = 0; s <= 10; s++) {
          const t = s / 10;
          points.push(new THREE.Vector3(-0.55 + t * 1.3, 0.2 + Math.sin(t * Math.PI * 1.7 + i * 0.8) * 0.06, zBias + Math.sin(t * 5 + i) * 0.03));
        }
    ...
    if (profile.eyeSegs > 0) {
      for (const side of [-1, 1]) {
        const socket = new THREE.Mesh(new THREE.SphereGeometry(0.18, profile.eyeSegs, profile.eyeSegs), socketMaterial);
        socket.scale.set(1.0, 0.9, 1.1);
        socket.position.set(0.86, 0.12, side * 0.42);
        tierGroup.add(socket);
        socketMeshes.push(socket);

        const remnantEye = new THREE.Mesh(new THREE.SphereGeometry(0.068, Math.max(6, profile.eyeSegs - 4), Math.max(6, profile.eyeSegs - 4)), eyeMaterial);
      ...
      for (const side of [-1, 1]) {
        const nasalCavity = new THREE.Mesh(new THREE.ConeGeometry(0.06, 0.2, 8), socketMaterial);
    ...
    const upperJaw = this._buildMandible(profile, false, boneMaterial, sutureMaterial, tierName);
    const lowerJaw = this._buildMandible(profile, true, boneMaterial, sutureMaterial, tierName);
    ...
    for (let i = 0; i < profile.membraneCount; i++) {
      ...
      const membraneGeo = new THREE.PlaneGeometry(width, height, profile.membraneSegments[0], profile.membraneSegments[1]);
      ...
      membraneMesh.position.set(-1.36 - i * 0.22, -0.18 - i * 0.19, (i - 1) * 0.42);
      membraneMesh.rotation.set(0.18 + i * 0.06, -0.1 - i * 0.06, (i - 1) * 0.14);
    ...
    if (profile.hornCount > 0) {
      for (let i = 0; i < profile.hornCount; i++) {
        ...
        const horn = new THREE.Mesh(new THREE.ConeGeometry(0.03, height, profile.hornSegments), hornMaterial);
        horn.position.set(0, -0.06 + Math.sin(t * Math.PI) * 0.22, (t - 0.5) * 1.3);
    ...
    for (let i = 0; i < profile.ghostCount; i++) {
      const light = new THREE.Mesh(new THREE.SphereGeometry(0.03, profile.ghostSegments, profile.ghostSegments), ghostMaterial.clone());
  ...
  _buildMandible(profile, isLower, boneMaterial, toothRootMaterial, tierName) {
    ...
    const halfCurve = new THREE.CatmullRomCurve3(curvePoints, false, 'catmullrom', 0.5);
    const leftHalf = new THREE.Mesh(new THREE.TubeGeometry(halfCurve, profile.jawTubularSegments, 0.05, profile.jawRadialSegments, false), boneMaterial);
    leftHalf.rotation.y = Math.PI;
    leftHalf.position.z = 0.53;
    group.add(leftHalf);

    const rightHalf = new THREE.Mesh(new THREE.TubeGeometry(halfCurve, profile.jawTubularSegments, 0.05, profile.jawRadialSegments, false), boneMaterial);
    rightHalf.position.z = -0.53;
    group.add(rightHalf);

    const frontBridge = new THREE.Mesh(new THREE.TorusGeometry(0.53, 0.045, profile.jawRadialSegments, profile.jawTubularSegments, Math.PI), boneMaterial);
    frontBridge.rotation.y = Math.PI / 2;
    frontBridge.position.set(0.54, -0.2 * jawSign, 0);
    group.add(frontBridge);

    const rearFlareL = new THREE.Mesh(new THREE.SphereGeometry(0.11, Math.max(8, profile.jawRadialSegments + 3), Math.max(7, profile.jawRadialSegments + 2)), boneMaterial);
    ...
    if (profile.toothCount > 0) {
      ...
      const toothGeo = new THREE.ConeGeometry(0.022, 0.13, profile.toothSegments);
      for (let i = 0; i < profile.toothCount; i++) {
        ...
        tooth.position.set(baseX + (isLower ? -0.005 : 0.005), -0.2 * jawSign + (isLower ? -0.06 : 0.06), baseZ);
  ...
  _deformSkull(geometry, includeMicroDetails) {
    ...
    for (let i = 0; i < p.count; i++) {
      ...
      const micro = includeMicroDetails ? Math.sin(y * 16 + z * 11 + x * 9) * 0.014 : 0;
  ...
  update(dt, playerPos) {
    ...
    if (this.turnTimer > this.turnInterval) {
      ...
      this.turnInterval = 9 + Math.random() * 12;
      if (Math.random() < 0.6) {
        this.desiredDirection.subVectors(playerPos, this.group.position).normalize();
        this.desiredDirection.y *= 0.35;
      } else {
        this.desiredDirection.set(Math.random() - 0.5, (Math.random() - 0.5) * 0.09, Math.random() - 0.5).normalize();
      }
    }

    this.direction.lerp(this.desiredDirection, dt * 0.45);
    _tmpVecA.copy(this.direction).multiplyScalar(this.speed * (0.7 + proximity * 0.35));
    this.velocity.lerp(_tmpVecA, dt * 1.8);
    this.group.position.addScaledVector(this.velocity, dt);
    this.group.position.y += Math.sin(this.time * 0.65 + this.bobPhase) * 0.26 * dt;

    const targetYaw = Math.atan2(this.direction.x, this.direction.z) + Math.PI / 2;
    this.group.rotation.y = THREE.MathUtils.lerp(this.group.rotation.y, targetYaw, dt * 1.2);

    _tmpVecB.subVectors(playerPos, this.group.position).normalize();
    const targetTilt = this.listeningTilt + _tmpVecB.y * 0.28;
    this.group.rotation.z = THREE.MathUtils.lerp(this.group.rotation.z, targetTilt * (0.3 + proximity * 0.9), dt * 0.65);
    ...
    if (shouldUpdateFar) {
      this._updateTierAnimation(this.tiers[lodTier], lodTier, dt, proximity, songPulse);
    }

    const pulseScale = 1 + (0.015 + proximity * 0.02) * Math.sin(this.time * 1.4 + this.songPhase);
  ...
  _selectLodTier(distanceToPlayer) {
    ...
    if (prev === 'near' && distanceToPlayer < SIREN_LOD_NEAR_DISTANCE + SIREN_LOD_HYSTERESIS) return prev;
    if (prev === 'medium' && distanceToPlayer > SIREN_LOD_NEAR_DISTANCE - SIREN_LOD_HYSTERESIS && distanceToPlayer < SIREN_LOD_MEDIUM_DISTANCE + SIREN_LOD_HYSTERESIS) return prev;
    if (prev === 'far' && distanceToPlayer > SIREN_LOD_MEDIUM_DISTANCE - SIREN_LOD_HYSTERESIS) return prev;

    this._lastLodTier = distanceToPlayer < SIREN_LOD_NEAR_DISTANCE
      ? 'near'
  ...
  _updateTierAnimation(tier, tierName, dt, proximity, songPulse) {
    ...
    tier.lowerJaw.rotation.z = THREE.MathUtils.lerp(tier.lowerJaw.rotation.z, jawBaseOpen + jawRhythm, dt * 2.5);
    tier.upperJaw.rotation.z = THREE.MathUtils.lerp(tier.upperJaw.rotation.z, 0.06 + Math.sin(this.time * 1.6 + 0.5) * 0.03 - proximity * 0.05, dt * 2.2);
    ...
    for (let i = 0; i < tier.horns.length; i++) {
      ...
      horn.scale.y = THREE.MathUtils.lerp(horn.scale.y, horn.userData.baseScaleY + proximity * 0.55, dt * 1.4);
    }

    for (let i = 0; i < tier.ghostLights.length; i++) {
      const ghost = tier.ghostLights[i];
      const drift = this.time * ghost.userData.orbitSpeed + ghost.userData.phase;
      ...
      mat.emissiveIntensity = tierName === 'near'
    ...
    for (let i = 0; i < tier.eyeRemnants.length; i++) {
      ...
      const flicker = 0.7 + Math.sin(this.time * (5.5 + i * 2.1)) * 0.28 + songPulse * 0.32;
      eye.material.emissiveIntensity = flicker;
      eye.scale.setScalar(0.95 + songPulse * 0.12);
    }

    tier.membraneMaterial.userData.shaderUniforms.uFlutterTime.value = this.time;
    tier.membraneMaterial.userData.shaderUniforms.uVelocity.value.copy(this.velocity);
    tier.membraneMaterial.userData.shaderUniforms.uPulse.value = songPulse;

    if (tierName !== 'far') {
      this._updateMembranesCpu(tier, dt, tier.profile.membraneCpuStep, proximity);
    }
  }
  ...
  _updateMembranesCpu(tier, dt, cpuStep, proximity) {
    for (let i = 0; i < tier.membranes.length; i++) {
      ...
      const velocityStretchY = Math.abs(this.velocity.y) * (0.06 + proximity * 0.04);
      ...
      for (let v = 0; v < position.count; v += cpuStep) {
        ...
        const propagation = Math.sin(this.time * (2.4 + i * 0.35) + trail * 8.5 + phase) * trail;
        const drag = Math.sin(this.time * 6.8 + u * 14.0 + i) * edge * trail;

        posArray[pIndex] = base[pIndex] + velocityStretchX * trail * 0.9;
        posArray[pIndex + 1] = base[pIndex + 1] - velocityStretchY * trail * 0.3;
        posArray[pIndex + 2] = base[pIndex + 2] + propagation * (0.09 + proximity * 0.05) + drag * 0.04;
      }

      position.needsUpdate = true;
      geometry.computeVertexNormals();

      membrane.position.x = membrane.userData.memData.originalX - this.velocity.length() * (0.06 + i * 0.01);
    }
  }
