...
| Three.js & WebGPU Guides | `threejs-webgpu-guides/`    | Curated guide index for Three.js rendering/materials/shadows and WebGPU topics, including WebGPU Fundamentals, Tour of WGSL, and toji.dev best practices for buffer uploads, bind groups, render bundles, and profiling      |
| Deep-Ocean Horror Design | `deep-ocean-horror-design/` | Design guidance for creatures, biomes, encounters, lighting, audio, pacing, HUD pressure, and environmental storytelling so new ideas stay grounded in deep-ocean horror instead of drifting into generic underwater fantasy |
